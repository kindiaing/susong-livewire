<div class="">
    ...
    <div class="flex items-center gap-3 mb-4">
        <div x-data class="relative">
            <input type="text" wire:model.live="search" class="flex h-9 w-64 rounded-md border border-input bg-background pl-3 pr-8 text-sm" placeholder="搜索标签名称..." />
            @if($search)
    ...
    <div class="rounded-lg border bg-card">
        <table class="w-full text-sm">
            ...
            <tbody>
                @forelse($tags as $tag)
                <tr class="border-b last:border-b-0 hover:bg-muted/30 transition-colors" wire:key="tag-{{ $tag->id }}">
                    <td class="px-4 py-2"><input type="checkbox" value="{{ $tag->id }}" wire:model.live="selectedIds" class="rounded" /></td>
                    @foreach($visibleCols as $col)
                    @switch($col['key'])
                        @case('id')
                            <td class="px-4 py-2 text-muted-foreground">{{ $tag->id }}</td>
                            @break
                        @case('name')
                            <td class="px-4 py-2 font-medium text-foreground">{{ $tag->name }}</td>
                            @break
                        @case('sort')
                            <td class="px-4 py-2 text-foreground">{{ $tag->sort }}</td>
                            @break
                        @case('status')
                            <td class="px-4 py-2">{!! status_badge($tag->status, 'active') !!}</td>
                            @break
                        @default
                            <td class="px-4 py-2 text-foreground">{{ $tag->{$col['key']} ?? '-' }}</td>
                    @endswitch
                    @endforeach
                    <td class="px-4 py-2">
                        <div class="flex items-center gap-2">
                            @can('product.tag.edit')
                            <button type="button" wire:click="openEditModal({{ $tag->id }})" class="p-1 rounded text-blue-600 hover:bg-blue-50 hover:text-blue-700 transition-colors" title="编辑"><x-ui.icon name="pencil-square" class="w-3.5 h-3.5" /></button>
                            @endcan
                            @can('product.tag.delete')
                            <button type="button" wire:click="confirmDelete({{ $tag->id }})" class="p-1 rounded text-red-600 hover:bg-red-50 hover:text-red-700 transition-colors" title="删除"><x-ui.icon name="trash" class="w-3.5 h-3.5" /></button>
                            @endcan
                        </div>
                    </td>
                </tr>
                @empty
                <tr><td colspan="{{ $colspan }}" class="px-6 py-12 text-center text-muted-foreground">暂无标签数据</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-background rounded-lg border shadow-lg w-full max-w-md mx-4 p-6">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-foreground">{{ $editingId ? '编辑标签' : '新增标签' }}</h2>
