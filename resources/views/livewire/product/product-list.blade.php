                ...
                <tr class="border-b last:border-b-0 hover:bg-muted/30 transition-colors" wire:key="product-{{ $product->id }}">
                    ...
                    <td class="px-4 py-2">
                        <div class="flex items-center gap-2">
                            ...
                            <button type="button" wire:click="openEditModal({{ $product->id }})" class="p-1 rounded text-blue-600 hover:bg-blue-50 hover:text-blue-700 transition-colors" title="编辑"><x-ui.icon name="pencil-square" class="w-3.5 h-3.5" /></button>
                            @endcan
                            @can('product.product.delete')
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-background rounded-lg border shadow-lg w-full max-w-lg mx-4 p-6 max-h-[85vh] overflow-y-auto">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-foreground">{{ $editingId ? '编辑商品' : '新增商品' }}</h2>
