        ...
        @forelse($carts as $cart)
            <div class="grid gap-3 border-b last:border-b-0 px-6 py-3 items-center hover:bg-muted/30 transition-colors"
                ...
                <div class="flex items-center gap-2">
                    @can('order.cart.edit')
                    <button type="button" wire:click="openEditModal({{ $cart->id }})" class="p-1 rounded text-blue-600 hover:bg-blue-50 hover:text-blue-700 transition-colors" title="编辑"><x-ui.icon name="pencil" class="w-3.5 h-3.5" /></button>
                    @endcan
                    @can('order.cart.delete')
