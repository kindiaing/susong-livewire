<div class="p-6">
    ...
    <div class="flex items-center gap-3 mb-4">
        <input type="text" wire:model.live="search" class="flex h-9 w-64 rounded-md border border-input bg-background px-3 text-sm" placeholder="搜索任务编号..." />
    ...
    <div class="rounded-lg border bg-card">
        <div class="grid grid-cols-[60px_1fr_120px_100px] gap-3 border-b px-6 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">
            <div>ID</div><div>任务编号</div><div>创建时间</div><div>操作</div>
        </div>
        @forelse($items as $item)
            <div class="grid grid-cols-[60px_1fr_120px_100px] gap-3 border-b last:border-b-0 px-6 py-3 items-center hover:bg-muted/30 transition-colors" wire:key="delivery-task-list-{{ $item->id }}">
