<div class="">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-foreground">温度记录</h1>
            <p class="text-muted-foreground mt-1">冷链配送温度监控</p>
        </div>
    </div>
    ...
    <div class="flex items-center gap-3 mb-4">
        <div x-data class="relative">
            <input type="text" wire:model.live="search" class="flex h-9 w-64 rounded-md border border-input bg-background pl-3 pr-8 text-sm" placeholder="搜索温度记录..." />
            @if($search)
        ...
        @if($selectedCount > 0)
            ...
            @can('delivery.temperature.delete')
    ...
    @php
        $allCols = collect($this->getAllColumns())
            ->filter(fn($col) => $col['key'] !== 'temperature')
            ->values();
    ...
    <div class="rounded-lg border bg-card">
        ...
        @forelse($items as $item)
            <div class="grid gap-3 border-b last:border-b-0 px-6 py-3 items-center hover:bg-muted/30 transition-colors"
                 ...
                 wire:key="temperature-{{ $item->id }}">
                <div><input type="checkbox" value="{{ $item->id }}" wire:model.live="selectedIds" class="rounded" /></div>
                <div class="text-sm font-medium text-foreground">{{ $item->temperature }}°C</div>
                @foreach($visibleCols as $col)
                    @switch($col['key'])
                        @case('id')
                            ...
                            @break
                        @case('delivery_id')
                            <div class="text-sm text-foreground">{{ $item->delivery_id }}</div>
                            @break
                        @case('recorded_at')
                            <div class="text-sm text-foreground">{{ $item->recorded_at?->format('Y-m-d H:i') ?? '-' }}</div>
                            @break
                        @case('created_at')
        ...
        @empty
            <div class="px-6 py-12 text-center text-sm text-muted-foreground">暂无温度记录</div>
        @endforelse
    </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-background rounded-lg border shadow-lg w-full max-w-sm mx-4 p-6">
            ...
            <p class="text-sm text-muted-foreground mb-6">确定要删除该温度记录吗？此操作不可恢复。</p>
