<div class="">
        ...
        @forelse($items as $item)
            <div class="grid gap-3 border-b last:border-b-0 px-6 py-3 items-center hover:bg-muted/30 transition-colors"
                ...
                <div class="flex items-center gap-2">
                    @can('finance.receivable.create')
                    @if($item->status !== 3)
                    <button type="button" wire:click="confirmReceived({{ $item->id }})" class="text-green-600 hover:text-green-700 text-sm">确认收款</button>
                    @endif
                    @endcan
                    @can('finance.receivable.delete')
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-background rounded-lg border shadow-lg w-full max-w-lg mx-4 p-6 max-h-[85vh] overflow-y-auto">
            <h2 class="text-lg font-semibold text-foreground mb-4">新增应收</h2>
            <div class="space-y-4">
                <div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-background rounded-lg border shadow-lg w-full max-w-sm mx-4 p-6">
            ...
            <p class="text-sm text-muted-foreground mb-6">确定要删除该应收记录吗？此操作不可恢复。</p>
