<div class="p-6">
    ...
    <div class="flex items-center gap-3 mb-4">
        <div x-data class="relative">
            <input type="text" wire:model.live="search" class="flex h-9 w-64 rounded-md border border-input bg-background pl-3 pr-8 text-sm" placeholder="搜索结算单号..." />
            @if($search)
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b text-xs font-medium text-muted-foreground uppercase tracking-wider">
                    ...
                    <th class="px-4 py-2 text-left">结算单号</th>
            ...
            <tbody>
                ...
                <tr class="border-b last:border-b-0 hover:bg-muted/30 transition-colors" wire:key="supplier-settlement-list-{{ $item->id }}">
                    <td class="px-4 py-2 text-muted-foreground">{{ $item->id }}</td>
                    <td class="px-4 py-2 text-foreground">{{ $item->settlement_no }}</td>
