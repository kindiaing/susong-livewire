<div class="">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-background rounded-lg border shadow-lg w-full max-w-lg mx-4 p-6 max-h-[85vh] overflow-y-auto">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-foreground">{{ $editingId ? '编辑发票' : '新增发票' }}</h2>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-background rounded-lg border shadow-lg w-full max-w-lg mx-4 p-6 max-h-[85vh] overflow-y-auto">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-foreground">发票详情</h2>
            ...
            <div class="space-y-3 text-sm">
                <div class="flex"><span class="w-24 text-muted-foreground">发票号</span><span class="text-foreground font-mono">{{ $detailItem->invoice_no ?? '-' }}</span></div>
                <div class="flex"><span class="w-24 text-muted-foreground">类型</span><span class="text-foreground">{{ $typeMap[$detailItem->type] ?? '-' }}</span></div>
                ...
                <div class="flex"><span class="w-24 text-muted-foreground">抬头</span><span class="text-foreground">{{ $detailItem->title ?? '-' }}</span></div>
                <div class="flex"><span class="w-24 text-muted-foreground">税号</span><span class="text-foreground">{{ $detailItem->tax_no ?? '-' }}</span></div>
                ...
                <div class="flex"><span class="w-24 text-muted-foreground">开具时间</span><span class="text-foreground">{{ $detailItem->issued_at ?? '-' }}</span></div>
                <div class="flex"><span class="w-24 text-muted-foreground">寄出时间</span><span class="text-foreground">{{ $detailItem->sent_at ?? '-' }}</span></div>
