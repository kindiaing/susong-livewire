<div class="">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-muted-foreground mt-1">管理价格策略配置</p>
        </div>
    </div>
    ...
    <div class="flex items-center gap-3 mb-4">
        <div x-data class="relative">
            <input type="text" wire:model.live="search" class="flex h-9 w-64 rounded-md border border-input bg-background pl-3 pr-8 text-sm" placeholder="搜索策略名称..." />
            @if($search)
    ...
    @php
        $statusMap = [0 => '禁用', 1 => '启用'];
        $statusColorMap = [0 => 'gray', 1 => 'green'];
        $typeMap = ['discount' => '折扣', 'fixed' => '固定价格', 'tier' => '阶梯价格', 'percentage' => '百分比加价'];
    ...
    <div class="rounded-lg border bg-card">
        ...
        @forelse($items as $item)
            <div class="grid gap-3 border-b last:border-b-0 px-6 py-3 items-center hover:bg-muted/30 transition-colors"
                 ...
                 wire:key="price-strategy-{{ $item->id }}">
        ...
        @empty
            <div class="px-6 py-12 text-center text-sm text-muted-foreground">暂无策略数据</div>
        @endforelse
    </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-background rounded-lg border shadow-lg w-full max-w-sm mx-4 p-6">
            ...
            <p class="text-sm text-muted-foreground mb-6">确定要删除该价格策略吗？此操作不可恢复。</p>
