    ...
    <div class="flex gap-6">
        ...
        <div class="w-48 shrink-0">
            <nav class="sticky top-6 space-y-1">
                @foreach($groups as $groupKey => $groupLabel)
                    <button
                        wire:click="setActiveGroup('{{ $groupKey }}')"
        ...
        <div class="flex-1">
            <div class="rounded-lg border bg-card">
                {{-- 列表头 --}}
                <div class="grid grid-cols-[1fr_200px_80px_120px] gap-4 border-b px-6 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">
                    <div>配置项</div>
                    <div>当前值</div>
                    <div>类型</div>
                ...
                @forelse($configs as $config)
                    <div
                        class="grid grid-cols-[1fr_200px_80px_120px] gap-4 border-b last:border-b-0 px-6 py-4 items-center hover:bg-muted/30 transition-colors"
                    ...
                    >
                        ...
                        <div class="flex justify-end gap-1">
                            ...
                            @else
                                @if(!$config->is_readonly)
                                    <button
                                        wire:click="startEdit({{ $config->id }})"
                                ...
                                @if($config->default_value && $config->config_value !== $config->default_value && !$config->is_readonly)
                                    <button
                                        wire:click="resetToDefault({{ $config->id }})"
