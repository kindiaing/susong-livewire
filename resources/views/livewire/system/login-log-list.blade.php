<div class="p-6">
    ...
    <div class="flex items-center gap-3 mb-4">
        <div x-data class="relative">
            <input type="text" wire:model.live="search" class="flex h-9 w-64 rounded-md border border-input bg-background pl-3 pr-8 text-sm" placeholder="搜索用户名..." />
            @if($search)
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b text-xs font-medium text-muted-foreground uppercase tracking-wider">
                    ...
                    <th class="px-4 py-2 text-left">UA</th>
                    <th class="px-4 py-2 text-left">创建时间</th>
                </tr>
            </thead>
            <tbody>
                @forelse($items as $item)
                <tr class="border-b last:border-b-0 hover:bg-muted/30 transition-colors" wire:key="login-log-list-{{ $item->id }}">
                    ...
                    <td class="px-4 py-2 text-foreground">{{ $item->user_id }}</td>
                    <td class="px-4 py-2 text-muted-foreground font-mono">{{ $item->ip }}</td>
                    <td class="px-4 py-2 text-muted-foreground truncate">{{ $item->user_agent }}</td>
