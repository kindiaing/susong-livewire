<div class="">
    ...
    <div class="flex items-center gap-3 mb-4">
        <div x-data class="relative">
            <input type="text" wire:model.live="search" class="flex h-9 w-64 rounded-md border border-input bg-background pl-3 pr-8 text-sm" placeholder="搜索姓名/手机号/身份证..." />
            @if($search)
            ...
            <div class="grid gap-3 border-b last:border-b-0 px-6 py-3 items-center hover:bg-muted/30 transition-colors"
                ...
                @foreach($visibleCols as $col)
                    @switch($col['key'])
                        ...
                        @case('online_status')
                            <div>
                                {!! status_badge($driver->online_status, 'driver_online') !!}
                            </div>
                            @break
                        @case('status')
                            <div>
                                {!! status_badge($driver->status, 'active') !!}
                            </div>
                            @break
                        @case('created_at')
                ...
                <div class="flex items-center gap-2">
                    @can('org.driver.edit')
                    <button type="button" wire:click="openEditModal({{ $driver->id }})" class="p-1 rounded text-blue-600 hover:bg-blue-50 hover:text-blue-700 transition-colors" title="编辑"><x-ui.icon name="pencil-square" class="w-3.5 h-3.5" /></button>
                    @endcan
                    @can('org.driver.delete')
                    <button type="button" wire:click="confirmDelete({{ $driver->id }})" class="p-1 rounded text-red-600 hover:bg-red-50 hover:text-red-700 transition-colors" title="删除"><x-ui.icon name="trash" class="w-3.5 h-3.5" /></button>
                    @endcan
                </div>
            </div>
        @empty
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-background rounded-lg border shadow-lg w-full max-w-md mx-4 p-6">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-foreground">{{ $editingId ? '编辑司机' : '新增司机' }}</h2>
