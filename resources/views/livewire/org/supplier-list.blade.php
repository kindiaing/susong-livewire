            ...
            <div class="grid gap-3 border-b last:border-b-0 px-6 py-3 items-center hover:bg-muted/30 transition-colors"
                ...
                @foreach($visibleCols as $col)
                    @switch($col['key'])
                        ...
                        @case('status')
                            <div>
                                {!! status_badge($supplier->status, 'active') !!}
                            </div>
                            @break
                        @case('created_at')
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button type="button" wire:click="openEditModal({{ $supplier->id }})" class="p-1 rounded text-blue-600 hover:bg-blue-50 hover:text-blue-700 transition-colors" title="编辑"><x-ui.icon name="pencil-square" class="w-3.5 h-3.5" /></button>
                    @endcan
                    @can('org.supplier.edit')
                    ...
                    @endcan
                    @can('org.supplier.delete')
                    <button type="button" wire:click="confirmDelete({{ $supplier->id }})" class="p-1 rounded text-red-600 hover:bg-red-50 hover:text-red-700 transition-colors" title="删除"><x-ui.icon name="trash" class="w-3.5 h-3.5" /></button>
                    @endcan
                </div>
            </div>
        @empty
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-background rounded-lg border shadow-lg w-full max-w-lg mx-4 p-6 max-h-[85vh] overflow-y-auto">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-foreground">{{ $editingId ? '编辑供应商' : '新增供应商' }}</h2>
