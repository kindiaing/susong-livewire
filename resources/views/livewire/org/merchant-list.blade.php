        ...
        @forelse($merchants as $merchant)
            <div class="grid gap-3 border-b last:border-b-0 px-6 py-3 items-center hover:bg-muted/30 transition-colors min-w-[900px]"
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button type="button" wire:click="openEditModal({{ $merchant->id }})" class="text-blue-600 hover:text-blue-700 text-sm">编辑</button>
                    @endcan
                    @can('org.merchant.delete')
                    <button type="button" wire:click="confirmDelete({{ $merchant->id }})" class="text-red-600 hover:text-red-700 text-sm">删除</button>
                    @endcan
                </div>
            </div>
        @empty
