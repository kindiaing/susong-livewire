                ...
                <tr class="border-b last:border-b-0 hover:bg-muted/30 transition-colors" wire:key="perm-{{ $perm->id }}">
                    ...
                    <td class="px-4 py-2">
                        <div class="flex items-center gap-1">
                            ...
                            <button type="button" wire:click="confirmDelete({{ $perm->id }})" class="p-1 rounded text-red-600 hover:bg-red-50 hover:text-red-700 transition-colors" title="删除"><x-ui.icon name="trash" class="w-3.5 h-3.5" /></button>
                            @endcan
                        </div>
                    </td>
                </tr>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-background rounded-lg border shadow-lg w-full max-w-md mx-4 p-6">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-foreground">{{ $editingId ? '编辑权限' : '新增权限' }}</h2>
