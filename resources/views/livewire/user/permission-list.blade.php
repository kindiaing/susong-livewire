<div class="p-6">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-muted-foreground mt-1">管理系统权限节点（模块/页面/按钮）</p>
        </div>
        ...
        @forelse($permissions as $perm)
            <div class="grid grid-cols-[60px_1fr_1fr_80px_100px_80px_120px] gap-3 border-b last:border-b-0 px-6 py-3 items-center hover:bg-muted/30 transition-colors"
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button wire:click="delete({{ $perm->id }})" class="text-red-600 hover:text-red-700 text-sm" onclick="return confirm('确定删除该权限？')">删除</button>
                </div>
            </div>
        @empty
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-background rounded-lg border shadow-lg w-full max-w-md mx-4 p-6">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-foreground mb-1">权限标识</label>
