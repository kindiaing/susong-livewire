<div>
    ...
    <div class="rounded-lg border bg-card">
        <table class="w-full text-sm">
            ...
            <tbody>
                ...
                <tr class="border-b last:border-b-0 hover:bg-muted/30 transition-colors" wire:key="role-{{ $role->id }}">
                    ...
                    @switch($col['key'])
                        ...
                        @case('display_name')
                            ...
                            @break
                        @case('guard_name')
                            <td class="px-4 py-2 text-muted-foreground font-mono">{{ $role->guard_name }}</td>
                            @break
                        @case('users_count')
                        ...
                        @case('permissions_count')
                            ...
                            @break
                        @case('description')
                            <td class="px-4 py-2 text-muted-foreground">{{ $role->description ?? '—' }}</td>
                            @break
                        @case('created_at')
                            <td class="px-4 py-2 text-muted-foreground">{{ $role->created_at?->format('Y-m-d H:i') }}</td>
                            @break
                        @default
                            <td class="px-4 py-2 text-foreground">{{ $role->{$col['key']} ?? '—' }}</td>
                    @endswitch
                    @endforeach
                    <td class="px-4 py-2">
                ...
                @empty
                <tr><td colspan="{{ $colspan }}" class="px-6 py-12 text-center text-muted-foreground">暂无角色数据</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-background rounded-lg border shadow-lg w-full max-w-md mx-4 p-6">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-foreground">{{ $editingId ? '编辑角色' : '新增角色' }}</h2>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-background rounded-lg border shadow-lg w-full max-w-4xl mx-4 p-6">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h2 class="text-lg font-semibold text-foreground">分配权限</h2>
                    <p class="text-sm text-muted-foreground mt-1">角色：{{ $permissionRoleName }}</p>
                </div>
                <button type="button" wire:click="closePermissionModal" class="text-muted-foreground hover:text-foreground transition-colors"><x-ui.icon name="x-mark" class="w-5 h-5" /></button>
            </div>
