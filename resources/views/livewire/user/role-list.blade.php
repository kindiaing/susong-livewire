                ...
                <tr class="border-b last:border-b-0 hover:bg-muted/30 transition-colors" wire:key="role-{{ $role->id }}">
                    ...
                    <td class="px-4 py-2">
                        <div class="flex items-center gap-1">
                            @can('user.role.edit')
                            @if(!$isSuperAdmin)
                                <button type="button" wire:click="openEditModal({{ $role->id }})" class="p-1 rounded text-blue-600 hover:bg-blue-50 hover:text-blue-700 transition-colors" title="编辑"><x-ui.icon name="pencil" class="w-3.5 h-3.5" /></button>
                            @endif
                            @endcan
                            @can('user.role.edit')
